// ---------------------------------------------------------------------------
// String literal pre-scanning and data segment helpers.
// These only need mod, stringLiterals, dataSegmentOffset, dataSegments.
// ---------------------------------------------------------------------------
...
export function allocStringLiteral(ctx: StringContext, value: string): number {
...
export function prescanStringLiterals(ctx: StringContext, module: ModuleDecl): void {
...
export function scanExprForStrings(ctx: StringContext, expr: Expr): void {
  switch (expr.kind) {
    case "StringLiteral":
    ...
    case "UnaryExpr":
      ...
      break;
    case "CallExpr":
      for (const arg of expr.args) scanExprForStrings(ctx, arg.value);
      if (expr.callee.kind !== "IdentifierExpr") scanExprForStrings(ctx, expr.callee);
      break;
    case "MatchExpr":
      scanExprForStrings(ctx, expr.scrutinee);
      for (const arm of expr.arms) {
        scanExprForStrings(ctx, arm.body);
        if (arm.pattern.kind === "LiteralPattern") scanExprForStrings(ctx, arm.pattern.value);
      }
      break;
    case "LetExpr":
    ...
    case "AssignmentExpr":
      ...
      break;
    case "BlockExpr":
      for (const stmt of expr.statements) scanExprForStrings(ctx, stmt);
      if (expr.result) scanExprForStrings(ctx, expr.result);
      break;
    case "MemberExpr":
      scanExprForStrings(ctx, expr.object);
      break;
    case "ListLiteral":
      for (const elem of expr.elements) scanExprForStrings(ctx, elem);
      break;
    case "RecordLiteral":
      for (const field of expr.fields) scanExprForStrings(ctx, field.value);
      break;
    case "LambdaExpr":

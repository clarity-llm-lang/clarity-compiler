...
function fieldSize(type: ClarityType): number {
  return _fieldSize(type);
}

function unionSize(variants: ClarityVariant[]): number {
  return _unionSize(variants);
}

function storeField(
  mod: binaryen.Module,
  basePtr: binaryen.ExpressionRef,
  offset: number,
...
export function resolveTypeRefWithTypeParams(
  ctx: CallsContext,
  node: TypeNode,
  typeParams: string[],
): ClarityType {
  ...
  return assertResolvedType(ctx.checker.resolveTypeRef(node), `type '${(node as any).name ?? "unknown"}'`);
}

export function generateCall(ctx: CallsContext, expr: CallExpr): binaryen.ExpressionRef {
...
export function generateMonomorphizedCall(
  ctx: CallsContext,
  expr: CallExpr,
  ...
  if (!ctx.generatedMonomorphs.has(monoName)) {
    ...
    const savedTypeVarSubst = (ctx as any).typeVarSubst;
    ...
    (ctx as any).typeVarSubst = bindings;
    ...
    (ctx as any).typeVarSubst = savedTypeVarSubst;
  }
...
export function generateIndirectCall(
  ctx: CallsContext,
  expr: CallExpr,
  local: LocalVar,
...
export function findConstructorType(
  ctx: CallsContext,
  name: string,
): { union: Extract<ClarityType, { kind: "Union" }>; variantIndex: number; variant: ClarityVariant } | null {
...
export function generateConstructorCall(
  ...
  stmts.push(
    ctx.mod.local.set(ptrLocal,
      ctx.mod.call("__alloc", [ctx.mod.i32.const(size)], binaryen.i32),
    ),
  );
...
export function tryGenerateListCall(ctx: CallsContext, name: string, expr: CallExpr): binaryen.ExpressionRef | null {
  switch (name) {
    case "length": {
    ...
    case "is_empty": {
      const listArg = ctx.generateExpr(expr.args[0].value);
      return ctx.mod.i64.eq(
        ctx.mod.call("list_length", [listArg], binaryen.i64),
...
export function tryGenerateMapCall(ctx: CallsContext, name: string, expr: CallExpr): binaryen.ExpressionRef | null {

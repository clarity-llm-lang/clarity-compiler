// ---------------------------------------------------------------------------
// Pure memory layout helpers — zero class dependency.
// These functions only take ClarityType/binaryen arguments, no class state.
// ---------------------------------------------------------------------------
...
export function fieldSize(type: ClarityType): number {
  switch (type.kind) {
    case "Int64": return 8;
...
export function fieldAlign(type: ClarityType): number {
  switch (type.kind) {
    case "Int64": return 8;
    case "Float64": return 8;
...
export function recordLayout(fields: Map<string, ClarityType>): { name: string; type: ClarityType; offset: number }[] {
...
export function recordSize(fields: Map<string, ClarityType>): number {
  let offset = 0;
...
export function unionSize(variants: ClarityVariant[]): number {
  let maxPayload = 0;

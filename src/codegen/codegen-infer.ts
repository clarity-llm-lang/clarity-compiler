...
// These need the CodeGenerator's mutable state (locals, allFunctions, etc.)
// ---------------------------------------------------------------------------
import type { Expr, FunctionDecl } from "../ast/nodes.js";
import type { ClarityType } from "../checker/types.js";
import {
  INT64, FLOAT64, BOOL, UNIT, STRING,
  substituteTypeVars,
...
export interface InferContext {
  readonly locals: Map<string, { index: number; wasmType: binaryen.Type; clarityType: ClarityType }>;
...
export function inferExprType(ctx: InferContext, expr: Expr): ClarityType {
  ...
  switch (expr.kind) {
    ...
    case "IdentifierExpr": {
      ...
      if (ctx.functionTableIndices.has(expr.name)) {
        ...
        if (fn) return inferFunctionType(ctx, fn);
      }
      return INT64;
    }
    ...
    case "BlockExpr": {
      if (expr.result) return inferExprType(ctx, expr.result);
      return UNIT;
    }
...
export function inferFunctionType(ctx: InferContext, decl: FunctionDecl): ClarityType {
  const params = decl.params.map(p =>
    assertResolvedType(ctx.checker.resolveTypeRef(p.typeAnnotation), `parameter '${p.name}' in '${decl.name}'`),
  );
...
export function inferFunctionReturnType(ctx: InferContext, name: string): ClarityType {
  if (name === ctx.currentFunction.name) {
    return assertResolvedType(
      ctx.checker.resolveTypeRef(ctx.currentFunction.returnType),
      `return type of '${name}'`,
    );
  }
...
export function inferWasmReturnType(ctx: InferContext, name: string): binaryen.Type {
...
export function findConstructorType(
  ctx: InferContext,
  name: string,
): { union: Extract<ClarityType, { kind: "Union" }>; variantIndex: number; variant: import("../checker/types.js").ClarityVariant } | null {
  for (const [, type] of ctx.allTypeDecls) {
    if (type.kind === "Union") {

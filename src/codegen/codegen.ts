...
import { substituteTypeVars } from "../checker/types.js";
...
import {
  allocStringLiteral as _allocStringLiteral,
  prescanStringLiterals as _prescanStringLiterals,
} from "./codegen-strings.js";
import {
  fieldSize as _fieldSize, fieldAlign as _fieldAlign,
  recordLayout as _recordLayout, recordSize as _recordSize,
  unionSize as _unionSize, storeField as _storeField, loadField as _loadField,
} from "./codegen-memory.js";
import {
  inferExprType as _inferExprType,
  inferFunctionType as _inferFunctionType,
  inferFunctionReturnType as _inferFunctionReturnType,
  inferWasmReturnType as _inferWasmReturnType,
  findConstructorType as _findConstructorType,
  assertResolvedType,
} from "./codegen-infer.js";
import { generateMatch as _generateMatch, type MatchContext } from "./codegen-match.js";
import {
  buildClosureStruct as _buildClosureStruct, getOrCreateWrapper as _getOrCreateWrapper,
  liftLambda as _liftLambda, type ClosureContext,
} from "./codegen-closures.js";
import {
  isTailRecursive as _isTailRecursive, generateTailRecursiveBody as _generateTailRecursiveBody,
  type TcoContext,
} from "./codegen-tco.js";
import {
  generateCall as _generateCall, generateConstructorCall as _generateConstructorCall,
  type CallsContext, type LocalVar,
} from "./codegen-calls.js";
...
function mkInferCtx(g: CodeGenerator) {
  return {
...
function mkCtx<T>(g: CodeGenerator, extra: Partial<T>): T {
  return new Proxy(g as any, {
    get(target, prop) {
      if (prop in extra!) return (extra as any)[prop];
      return target[prop as keyof typeof target];
    },
    set(target, prop, val) {
      if (prop in extra!) { (extra as any)[prop] = val; return true; }
      (target as any)[prop] = val; return true;
    },
  }) as T;
}
...
export class CodeGenerator {
  ...
  stringLiterals: Map<string, number> = new Map();
  dataSegmentOffset: number = 0;
  dataSegments: { offset: number; data: Uint8Array }[] = [];

  allFunctions: Map<string, FunctionDecl> = new Map();
  allTypeDecls: Map<string, ClarityType> = new Map();

  functionTableNames: string[] = [];
  functionTableIndices: Map<string, number> = new Map();

  currentModuleWasmNames: Map<string, string> = new Map();
  functionDeclWasmNames: Map<FunctionDecl, string> = new Map();

  lambdaCounter: number = 0;
  pendingLambdas: Array<{ name: string; expr: import("../ast/nodes.js").LambdaExpr }> = [];
  lambdaWrappers: Map<string, number> = new Map();

  generatedMonomorphs: Set<string> = new Set();
  typeVarSubst: Map<string, ClarityType> = new Map();

  // Callbacks wired into the delegated context objects.
  private _genExpr = (e: Expr, et?: ClarityType) => this.generateExpr(e, et);
  private _inferExpr = (e: Expr) => this.inferExprType(e);
  private _inferWasmRet = (n: string) => this.inferWasmReturnType(n);
  private _allocStr = (v: string) => this.allocStringLiteral(v);

  private _mkMatchCtx(): MatchContext {
    return mkCtx<MatchContext>(this, {
      generateExpr: this._genExpr, inferExprType: this._inferExpr, allocStringLiteral: this._allocStr,
    });
  }
  private _mkClosureCtx(): ClosureContext {
    return mkCtx<ClosureContext>(this, {
      generateExpr: this._genExpr, inferExprType: this._inferExpr,
    });
  }
  private _mkTcoCtx(): TcoContext {
    return mkCtx<TcoContext>(this, {
      generateExpr: this._genExpr, inferExprType: this._inferExpr, allocStringLiteral: this._allocStr,
    });
  }
  private _mkCallsCtx(): CallsContext {
    return mkCtx<CallsContext>(this, {
      generateExpr: this._genExpr, inferExprType: this._inferExpr,
  ...
  generate(module: ModuleDecl, checker: Checker): Uint8Array {
    this._reset(checker); this.setupModule(module);
    if (!this.mod.validate()) throw new Error("Generated invalid WASM module");
    this.mod.optimize(); return this.mod.emitBinary();
  }
  generateText(module: ModuleDecl, checker: Checker): string {
    this._reset(checker); this.setupModule(module); this.mod.validate(); return this.mod.emitText();
  }
  generateMulti(allModules: ModuleDecl[], entryModule: ModuleDecl, checker: Checker): Uint8Array {
    this._reset(checker); this.setupModuleMulti(allModules, entryModule);
    if (!this.mod.validate()) throw new Error("Generated invalid WASM module");
    this.mod.optimize(); return this.mod.emitBinary();
  }
  generateTextMulti(allModules: ModuleDecl[], entryModule: ModuleDecl, checker: Checker): string {
    this._reset(checker); this.setupModuleMulti(allModules, entryModule);
    this.mod.validate(); return this.mod.emitText();
  }

  private _reset(checker: Checker): void {
    this.mod = new binaryen.Module(); this.checker = checker;
    this.stringLiterals = new Map(); this.dataSegmentOffset = 0; this.dataSegments = [];
    this.allFunctions = new Map(); this.allTypeDecls = new Map();
    this.functionTableNames = []; this.functionTableIndices = new Map();
    this.currentModuleWasmNames = new Map(); this.functionDeclWasmNames = new Map();
    this.generatedMonomorphs = new Set(); this.lambdaCounter = 0;
    this.pendingLambdas = []; this.lambdaWrappers = new Map(); this.typeVarSubst = new Map();
  }
  ...
  private setupModule(module: ModuleDecl): void {
    ...
    for (const decl of module.declarations) {
      if (decl.kind === "TypeDecl") {
        ...
        if (r) this.allTypeDecls.set(decl.name, r);
      }
      if (decl.kind === "FunctionDecl") this.allFunctions.set(decl.name, decl);
    }
    ...
    for (const decl of module.declarations) {
      if (decl.kind === "FunctionDecl" && decl.typeParams.length === 0) this.generateFunction(decl);
    }
    this._finalizeTable();
  ...
  private setupModuleMulti(allModules: ModuleDecl[], entryModule: ModuleDecl): void {
    ...
    this._registerCheckerTypes();
    for (const m of allModules) _prescanStringLiterals(this, m);
    this._setMemory();
    ...
    const entryNames = new Set<string>(
      entryModule.declarations.filter(d => d.kind === "FunctionDecl").map(d => (d as FunctionDecl).name),
    );
    for (const m of allModules) {
      this.currentModuleWasmNames = new Map(
        m.declarations.filter(d => d.kind === "FunctionDecl")
          .map(d => [d.name, this.functionDeclWasmNames.get(d as FunctionDecl)!]),
      );
      for (const d of m.declarations) {
        if (d.kind === "FunctionDecl" && d.typeParams.length === 0)
          this.generateFunctionMulti(d as FunctionDecl, entryNames);
      }
    }
    this._finalizeTable();
    this.mod.addGlobal("__heap_base", binaryen.i32, false, this.mod.i32.const(this.dataSegmentOffset || 1024));
  ...
  private _scanSigTypes(decls: import("../ast/nodes.js").Declaration[]): void {
    for (const d of decls) {
      if (d.kind === "FunctionDecl") {
        for (const tn of [...d.params.map(p => p.typeAnnotation), d.returnType]) {
          const r = this.checker.resolveTypeRef(tn);
          if (r && r.kind === "Union" && !this.allTypeDecls.has(r.name)) this.allTypeDecls.set(r.name, r);
          if (r) this._regNested(r);
        }
      }
    }
  }

  private _registerCheckerTypes(): void {
    for (const [n, t] of this.checker.getOptionTypes()) if (!this.allTypeDecls.has(n)) this.allTypeDecls.set(n, t);
    for (const [, t] of this.checker.getResultTypes())
      if (t.kind === "Union" && !this.allTypeDecls.has(t.name)) this.allTypeDecls.set(t.name, t);
  }

  private _regNested(type: ClarityType): void {
    if (!type) return;
    switch (type.kind) {
      case "Result": { const u = this.checker.resultToUnion(type); if (!this.allTypeDecls.has(u.name)) this.allTypeDecls.set(u.name, u); this._regNested(type.ok); this._regNested(type.err); break; }
      case "List": this._regNested(type.element); break;
      case "Option": this._regNested(type.inner); break;
      case "Union": for (const v of type.variants) for (const ft of v.fields.values()) this._regNested(ft); break;
      case "Record": for (const ft of type.fields.values()) this._regNested(ft); break;
      case "Function": for (const pt of type.params) this._regNested(pt); this._regNested(type.returnType); break;
      case "Map": this._regNested(type.key); this._regNested(type.value); break;
    }
  }

  private _setMemory(): void {
    this.mod.setMemory(1, 256, "memory",
      this.dataSegments.map(s => ({ name: `str_${s.offset}`, offset: this.mod.i32.const(s.offset), data: s.data, passive: false })),
    );
  }

  private _finalizeTable(): void {
  ...
  private fieldSize(t: ClarityType) { return _fieldSize(t); }
  private fieldAlign(t: ClarityType) { return _fieldAlign(t); }
  private recordLayout(f: Map<string, ClarityType>) { return _recordLayout(f); }
  private recordSize(f: Map<string, ClarityType>) { return _recordSize(f); }
  private unionSize(v: ClarityVariant[]) { return _unionSize(v); }
  private storeField(p: binaryen.ExpressionRef, o: number, v: binaryen.ExpressionRef, t: ClarityType) { return _storeField(this.mod, p, o, v, t); }
  private loadField(p: binaryen.ExpressionRef, o: number, t: ClarityType) { return _loadField(this.mod, p, o, t); }
  private allocStringLiteral(v: string) { return _allocStringLiteral(this, v); }

  private _genFuncBody(decl: FunctionDecl, ret: ClarityType, retWasm: binaryen.Type): binaryen.ExpressionRef {
  ...
  private generateFunction(decl: FunctionDecl): void {
    this.currentFunction = decl; this.locals = new Map(); this.localIndex = 0; this.additionalLocals = [];
    const pts: binaryen.Type[] = [];
    for (const p of decl.params) {
      ...
      pts.push(clarityTypeToWasm(ct)); this.localIndex++;
    }
  ...
  private generateFunctionMulti(decl: FunctionDecl, entryNames: Set<string>): void {
    this.currentFunction = decl; this.locals = new Map(); this.localIndex = 0; this.additionalLocals = [];
    const pts: binaryen.Type[] = [];
    for (const p of decl.params) {
      const ct = assertResolvedType(this.checker.resolveTypeRef(p.typeAnnotation), `parameter '${p.name}' in '${decl.name}'`);
    ...
    if (entryNames.has(decl.name)) this.mod.addFunctionExport(wn, decl.name);
  }
  ...
  private generateExpr(expr: Expr, expectedType?: ClarityType): binaryen.ExpressionRef {
    switch (expr.kind) {
      ...
      case "IdentifierExpr": {
        ...
        if (loc) return this.mod.local.get(loc.index, loc.wasmType);
        const ci = _findConstructorType(mkInferCtx(this), expr.name);
        if (ci && ci.variant.fields.size === 0) return _generateConstructorCall(this._mkCallsCtx(), expr.name, ci, []);
      ...
      case "LetExpr": {
        ...
        if (expr.name === "_") return this.mod.drop(val);
      ...
      case "AssignmentExpr": {
        const loc = this.locals.get(expr.name);
        if (!loc) throw new Error(`Undefined variable in codegen: ${expr.name}`);
      ...
      case "BlockExpr": {
        ...
        if (expr.result) stmts.push(this.generateExpr(expr.result, expectedType));
  ...
  private generateRecordLiteral(expr: import("../ast/nodes.js").RecordLiteral): binaryen.ExpressionRef {
    ...
    if (rt.kind !== "Record") throw new Error("Record literal did not resolve to a Record type");
    const layout = this.recordLayout(rt.fields);
    const pl = this.localIndex++; this.additionalLocals.push(binaryen.i32);
    const gp = () => this.mod.local.get(pl, binaryen.i32);
    const s: binaryen.ExpressionRef[] = [
    ...
    for (const f of expr.fields) {
      ...
      if (!le) throw new Error(`Record field '${f.name}' not found in layout`);
      s.push(this.storeField(gp(), le.offset, this.generateExpr(f.value), le.type));
    }
    s.push(gp()); return this.mod.block(null, s, binaryen.i32);
  }
  ...
  private generateMemberAccess(expr: import("../ast/nodes.js").MemberExpr): binaryen.ExpressionRef {
    ...
    if (ot.kind === "Record") {
      ...
      if (!f) throw new Error(`Record has no field '${expr.member}'`);
  ...
  private generateBinary(op: BinaryOp, left: Expr, right: Expr): binaryen.ExpressionRef {
    ...
    if (lt.kind === "Int64") {
      switch (op) {
        case "+": return this.mod.i64.add(l, r); case "-": return this.mod.i64.sub(l, r);
        case "*": return this.mod.i64.mul(l, r); case "/": return this.mod.i64.div_s(l, r);
        case "%": return this.mod.i64.rem_s(l, r); case "==": return this.mod.i64.eq(l, r);
        case "!=": return this.mod.i64.ne(l, r); case "<": return this.mod.i64.lt_s(l, r);
        case ">": return this.mod.i64.gt_s(l, r); case "<=": return this.mod.i64.le_s(l, r);
        case ">=": return this.mod.i64.ge_s(l, r);
      }
    }
    if (lt.kind === "Float64") {
      switch (op) {
        case "+": return this.mod.f64.add(l, r); case "-": return this.mod.f64.sub(l, r);
        case "*": return this.mod.f64.mul(l, r); case "/": return this.mod.f64.div(l, r);
        case "%": return this.mod.call("f64_rem", [l, r], binaryen.f64);
        case "==": return this.mod.f64.eq(l, r); case "!=": return this.mod.f64.ne(l, r);
        case "<": return this.mod.f64.lt(l, r); case ">": return this.mod.f64.gt(l, r);
        case "<=": return this.mod.f64.le(l, r); case ">=": return this.mod.f64.ge(l, r);
      }
    }
    if (lt.kind === "Bool") {
      switch (op) {
        case "and": return this.mod.i32.and(l, r); case "or": return this.mod.i32.or(l, r);
        case "==": return this.mod.i32.eq(l, r); case "!=": return this.mod.i32.ne(l, r);
      }
    }
  ...
  private generateUnary(op: UnaryOp, operand: Expr): binaryen.ExpressionRef {
    ...
    if (op === "-") {
      if (t.kind === "Int64") return this.mod.i64.sub(this.mod.i64.const(0, 0), e);
      if (t.kind === "Float64") return this.mod.f64.neg(e);
    }
    if (op === "!" && t.kind === "Bool") return this.mod.i32.xor(e, this.mod.i32.const(1));
    throw new Error(`Unsupported unary op '${op}' for type ${t.kind}`);
  }

  private inferExprType(e: Expr): ClarityType { return _inferExprType(mkInferCtx(this), e); }
  private inferFunctionType(d: FunctionDecl): ClarityType { return _inferFunctionType(mkInferCtx(this), d); }
  private inferFunctionReturnType(n: string): ClarityType { return _inferFunctionReturnType(mkInferCtx(this), n); }
  private inferWasmReturnType(n: string): binaryen.Type { return _inferWasmReturnType(mkInferCtx(this), n); }
}

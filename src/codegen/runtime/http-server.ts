...
export function createHttpServerRuntime(h: SharedHelpers) {
  ...
  return {
    ...
    http_next_request(handleN: bigint): number {
      ...
      if (!session) return h.allocResultString(false, h.writeString(`Unknown http server handle: ${handle}`));
      const { reqCtrl, reqSab, worker } = session;
      Atomics.wait(reqCtrl, 0, 0);
      const state = Atomics.load(reqCtrl, 0);
      if (state !== 1) return h.allocResultString(false, h.writeString("http_next_request: unexpected SAB state"));
      const reqMeta = new DataView(reqSab);

...
export function collectFreeVars(expr: Expr, bound: Set<string>): Set<string> {
  const free = new Set<string>();
  walkExpr(expr, bound, free);
  return free;
}

export function walkExpr(expr: Expr, bound: Set<string>, free: Set<string>): void {
  switch (expr.kind) {
    case "IdentifierExpr":
...
export function collectPatternBindings(pattern: Pattern, bound: Set<string>): void {
  switch (pattern.kind) {
    case "BindingPattern":

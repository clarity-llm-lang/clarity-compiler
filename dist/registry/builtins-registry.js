...
// -----------------------------------------------------------------------------
const LIST_INT = { kind: "List", element: INT64 };
const LIST_STRING = { kind: "List", element: STRING };
const MAP_STRING_STRING = { kind: "Map", key: STRING, value: STRING };
const OPTION_STRING = {
    kind: "Union",
    name: "Option<String>",
    variants: [
...
const OPTION_MAP_STRING_STRING = {
    kind: "Union",
...
const T = { kind: "TypeVar", name: "T" };
const LIST_T = { kind: "List", element: T };
// Generic type variables for Map<K, V>
const K = { kind: "TypeVar", name: "K" };
const V = { kind: "TypeVar", name: "V" };
const MAP_KV = { kind: "Map", key: K, value: V };
const LIST_K = { kind: "List", element: K };
const LIST_V = { kind: "List", element: V };
// -----------------------------------------------------------------------------
...
export const CLARITY_BUILTINS = [
    ...
    // --- HTTP server operations (require Network effect) ---
    {
        ...
        effects: ["Network"],
        doc: "Start an HTTP server on the given port. Returns Ok(handle) on success or Err(message) if the port is unavailable. Use the handle with http_next_request, http_respond, and http_close_server. Example: match http_listen(8080) { Ok(h) -> h, Err(e) -> ... }.",
        category: "network",
    },
    {
        name: "http_next_request",
        params: [INT64],
        paramNames: ["handle"],
        returnType: { kind: "Result", ok: STRING, err: STRING },
        effects: ["Network"],
        doc: "Block until the next HTTP request arrives on the server identified by handle. Returns Ok(request_json) where request_json is a JSON object with fields: id (request identifier Int64), method (String), path (String), query (String, includes leading ?), headers (JSON object), body (String). Use the id with http_respond or http_start_sse.",
        category: "network",
    },
    {
        name: "http_respond",
        params: [INT64, INT64, STRING, STRING],
        paramNames: ["request_id", "status", "headers_json", "body"],
        returnType: UNIT,
        effects: ["Network"],
        doc: "Send an HTTP response for a request. request_id is the id field from the http_next_request JSON. status is the HTTP status code (e.g. 200). headers_json is a JSON object of response headers — pass \"{}\" for defaults. body is the response body string.",
        category: "network",
    },
    {
        name: "http_close_server",
        params: [INT64],
        paramNames: ["handle"],
        returnType: UNIT,
        effects: ["Network"],
        doc: "Shut down an HTTP server started with http_listen and release the port.",
        category: "network",
    },
    {
        name: "http_start_sse",
        params: [INT64, STRING],
        paramNames: ["request_id", "headers_json"],
        returnType: UNIT,
        effects: ["Network"],
        doc: "Start a Server-Sent Events (SSE) stream for an HTTP request. Sends the SSE response headers and keeps the connection open. Use http_send_sse_event to push events and http_close_sse to close the stream. headers_json adds extra response headers (pass \"{}\" for defaults).",
        category: "network",
    },
    {
        name: "http_send_sse_event",
        params: [INT64, STRING],
        paramNames: ["request_id", "event_data"],
        returnType: UNIT,
        effects: ["Network"],
        doc: "Send a Server-Sent Events data event. event_data is the payload (the 'data:' line content). Requires a prior http_start_sse call for the same request_id.",
        category: "network",
    },
    {
        name: "http_close_sse",
        ...
        returnType: UNIT,
        effects: ["Network"],
        doc: "Close an SSE stream and send the final response. Terminates the connection started by http_start_sse for the given request_id.",
        category: "network",
    },
    ...
    // --- TTY builtins (require TTY effect unless noted) ---
    {
        name: "tty_is_tty",
        params: [],
        paramNames: [],
        returnType: BOOL,
        effects: [],
        doc: "Return True if stdout is connected to a real TTY (interactive terminal). Pure — no effect required. Use this to decide whether to use cursor/raw-mode features or fall back to plain output.",
        category: "tty",
    },
    {
        name: "tty_term_width",
        ...
        returnType: INT64,
        effects: ["TTY"],
        doc: "Return the current terminal width in columns. Returns 80 if not a TTY or width is unavailable.",
        category: "tty",
    },
    {
        name: "tty_term_height",
        params: [],
        paramNames: [],
        returnType: INT64,
        effects: ["TTY"],
        doc: "Return the current terminal height in rows. Returns 24 if not a TTY or height is unavailable.",
        category: "tty",
    },
    {
        name: "tty_enter_raw",
        ...
        returnType: UNIT,
        effects: ["TTY"],
        doc: "Enable raw (character-at-a-time) mode on stdin. After this call, keypresses are delivered immediately without waiting for Enter, and line editing is disabled. Restore with tty_exit_raw(). Has no effect if stdin is not a TTY.",
        category: "tty",
    },
    {
        name: "tty_exit_raw",
        ...
        returnType: UNIT,
        effects: ["TTY"],
        doc: "Disable raw mode and restore normal line-buffered stdin. Always call this before your program exits if you called tty_enter_raw().",
        category: "tty",
    },
    {
        name: "tty_read_key",
        ...
        returnType: OPTION_STRING,
        effects: ["TTY"],
        doc: "Read one keypress from stdin (requires raw mode via tty_enter_raw). Blocks up to timeout_ms milliseconds. Returns Some(key) where key is one of: \"up\", \"down\", \"left\", \"right\", \"enter\", \"space\", \"backspace\", \"escape\", \"ctrl+c\", \"ctrl+d\", or a single printable character. Returns None on timeout or EOF.",
        category: "tty",
    },
    {
        name: "tty_cursor_up",
        ...
        returnType: UNIT,
        effects: ["TTY"],
        doc: "Move the cursor up n lines using ANSI escape codes. Has no effect if n <= 0.",
        category: "tty",
    },
    {
        name: "tty_cursor_down",
        params: [INT64],
        paramNames: ["n"],
        returnType: UNIT,
        effects: ["TTY"],
        doc: "Move the cursor down n lines using ANSI escape codes. Has no effect if n <= 0.",
        category: "tty",
    },
    {
        name: "tty_cursor_to_col",
        params: [INT64],
        paramNames: ["col"],
        returnType: UNIT,
        effects: ["TTY"],
        doc: "Move the cursor to column col on the current line (1-based). Use col=1 to return to the start of the line.",
        category: "tty",
    },
    {
        name: "tty_clear_line",
        ...
        returnType: UNIT,
        effects: ["TTY"],
        doc: "Clear the current line and move cursor to column 1. Equivalent to \x1b[2K\r. Use for in-place line redraw.",
        category: "tty",
    },
    {
        name: "tty_hide_cursor",
        params: [],
        paramNames: [],
        returnType: UNIT,
        effects: ["TTY"],
        doc: "Hide the terminal cursor. Remember to call tty_show_cursor() before your program exits.",
        category: "tty",
    },
    {
        name: "tty_show_cursor",
        params: [],
        paramNames: [],
        returnType: UNIT,
        effects: ["TTY"],
        doc: "Show the terminal cursor (restores visibility after tty_hide_cursor).",
        category: "tty",
    },
    // --- Mux builtins (multi-stream SSE fan-in) ---
    {
        ...
        returnType: INT64,
        effects: [],
        doc: "Create a new stream multiplexer. Returns a handle for use with mux_add, mux_next, mux_remove, and mux_close. The mux is idle until streams are added with mux_add. No effect required — the mux is just a local handle.",
        category: "network",
    },
    {
        name: "mux_add",
        params: [INT64, STRING, STRING, STRING],
        paramNames: ["handle", "stream_id", "url", "headers_json"],
        returnType: UNIT,
        effects: ["Network"],
        doc: "Connect an SSE stream to the mux. stream_id is a caller-assigned label returned with each event from mux_next. url is the SSE endpoint. headers_json is a JSON object of request headers (pass \"{}\" for defaults). Events from this stream will be available via mux_next.",
        category: "network",
    },
    {
        name: "mux_next",
        ...
        returnType: OPTION_STRING,
        effects: ["Network"],
        doc: "Block until an event arrives from any stream in the mux, or until timeout_ms elapses. Returns Some(event_json) where event_json is a JSON object with fields: id (stream_id String), event (String event data, or null if the stream ended/errored), ended (Bool, true if stream closed normally), error (String, present if stream errored). Returns None on timeout.",
        category: "network",
    },
    {
        name: "mux_remove",
        params: [INT64, STRING],
        paramNames: ["handle", "stream_id"],
        returnType: UNIT,
        effects: ["Network"],
        doc: "Disconnect and remove a stream from the mux. The stream_id must match one passed to mux_add. In-flight events from this stream may still arrive via mux_next briefly after removal.",
        category: "network",
    },
    {
        name: "mux_close",
        ...
        returnType: UNIT,
        effects: ["Network"],
        doc: "Close all streams in the mux and release the handle. After this call the handle is invalid.",
        category: "network",
    },

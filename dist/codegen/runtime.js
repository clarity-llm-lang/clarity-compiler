...
`;
// ─────────────────────────────────────────────────────────────────────────────
// TTY: Raw key input via persistent worker + Atomics.
//
// SAB layout (40 bytes):
//   [0..3]  Int32  state: 0=IDLE, 1=KEY_READY, 2=EOF, 3=ERROR
//   [4..7]  Int32  key string byte length
//   [8..39] key name UTF-8 (up to 32 bytes)
// ─────────────────────────────────────────────────────────────────────────────
const TTY_KEY_SAB_SIZE = 40;
const TTY_KEY_OFFSET = 8;
const TTY_KEY_MAX = 32;
const _TTY_KEY_WORKER_CODE = `
const { workerData } = require('worker_threads');
const fs = require('fs');
...
function readNext() {
  ...
  if (!key) { readNext(); return; } // skip unrecognized sequences
  ...
  Atomics.notify(ctrl, 0, 1);
  // Wait until consumer resets status back to IDLE before reading next key.
...
`;
// ─────────────────────────────────────────────────────────────────────────────
// Mux: multi-stream SSE fan-in via single worker + Atomics.
//
// SAB layout (8 bytes header + MUX_MAX_EVENT bytes body):
//   [0..3]  Int32  state: 0=IDLE, 1=EVENT_READY
//   [4..7]  Int32  event JSON byte length
//   [8..]   event JSON UTF-8
// ─────────────────────────────────────────────────────────────────────────────
const MUX_MAX_EVENT = 65536; // 64 KB max event JSON
const MUX_SAB_SIZE = 8 + MUX_MAX_EVENT;
const _MUX_WORKER_CODE = `
const { parentPort, workerData } = require('worker_threads');
const https = require('https');
const http = require('http');
...
// Active streams: stream_id (string) -> { req, destroyed }
var streams = new Map();
...
export function createRuntime(config = {}) {
    ...
    function free(ptr) {
        ...
        if (cls === undefined)
            return; // not a tracked allocation; ignore
    ...
    let stdinReaderSab = null;
    let stdinReaderCtrl = null;
    let stdinReaderWorker = null;
    ...
    let nextSseHandle = 1;
    // TTY key reader: persistent worker + SAB for raw keypress delivery
...
`;
    ...
    const imports = {
        env: {
            ...
            bytes_set(ptr, index, value) {
                ...
                new Uint8Array(memory.buffer, newPtr + 4, len).set(new Uint8Array(memory.buffer, ptr + 4, len));
                if (i >= 0 && i < len) {
            ...
            bytes_slice(ptr, start, length) {
                ...
                new Uint8Array(memory.buffer, newPtr + 4, l).set(new Uint8Array(memory.buffer, ptr + 4 + s, l));
                return newPtr;
            },
            bytes_concat(aPtr, bPtr) {
                ...
                new Uint8Array(memory.buffer, newPtr + 4, aLen).set(new Uint8Array(memory.buffer, aPtr + 4, aLen));
                new Uint8Array(memory.buffer, newPtr + 4 + aLen, bLen).set(new Uint8Array(memory.buffer, bPtr + 4, bLen));
                return newPtr;
            },
            bytes_from_string(strPtr) {
                ...
                new Uint8Array(memory.buffer, newPtr, totalSize).set(new Uint8Array(memory.buffer, strPtr, totalSize));
                return newPtr;
            },
            ...
            json_get_path(jsonPtr, pathPtr) {
                try {
                    let current = JSON.parse(readString(jsonPtr));
            ...
            json_get_nested(jsonPtr, pathPtr) {
                try {
                    let node = JSON.parse(readString(jsonPtr));
            ...
            arena_restore_keeping_str(mark, strPtr) {
                ...
                if (markPtr < heapPtr) {
                    ...
                    for (const [s, p] of internedStrings) {
                        if (p >= markPtr)
                            internedStrings.delete(s);
                    }
                    for (const [p] of allocSizeMap) {
                        if (p >= markPtr)
                            allocSizeMap.delete(p);
                    }
                    for (const [cls, list] of freeLists) {
                        const trimmed = list.filter(p => p < markPtr);
                        if (trimmed.length !== list.length)
            ...
            list_tail(ptr, elemSize) {
                ...
                new Uint8Array(memory.buffer, newPtr + 4, newLen * elemSize).set(new Uint8Array(memory.buffer, ptr + 4 + elemSize, newLen * elemSize));
                return newPtr;
            },
            list_append_i64(ptr, value) {
                ...
                new Uint8Array(memory.buffer, newPtr + 4, len * 8).set(new Uint8Array(memory.buffer, ptr + 4, len * 8));
                // Append new element
            ...
            list_append_i32(ptr, value) {
                ...
                const newView = new DataView(memory.buffer);
                newView.setInt32(newPtr, newLen, true);
                new Uint8Array(memory.buffer, newPtr + 4, len * 4).set(new Uint8Array(memory.buffer, ptr + 4, len * 4));
            ...
            list_concat(aPtr, bPtr, elemSize) {
                ...
                newView.setInt32(newPtr, newLen, true);
                // Copy a elements
                new Uint8Array(memory.buffer, newPtr + 4, aLen * elemSize).set(new Uint8Array(memory.buffer, aPtr + 4, aLen * elemSize));
                // Copy b elements
                new Uint8Array(memory.buffer, newPtr + 4 + aLen * elemSize, bLen * elemSize).set(new Uint8Array(memory.buffer, bPtr + 4, bLen * elemSize));
                return newPtr;
            },
            // --- Map operations ---
            ...
            map_get_str_i32(handle, keyPtr) {
                ...
                if (!m?.has(key))
                    return allocOptionI32(null);
                return allocOptionI32(m.get(key));
            },
            map_get_str_i64(handle, keyPtr) {
                ...
                if (!m?.has(key))
                    return allocOptionI64(null);
                return allocOptionI64(m.get(key));
            },
            ...
            map_keys_str(handle) {
                ...
                if (!m)
                    return allocListI32([]);
                return allocListI32([...m.keys()].map((k) => writeString(k)));
            },
            map_values_i32(handle) {
                const m = mapTable.get(handle);
                if (!m)
                    return allocListI32([]);
                return allocListI32([...m.values()]);
            },
            ...
            map_keys_i64(handle) {
                ...
                if (!m)
                    return allocListI64([]);
                return allocListI64([...m.keys()]);
            },
            map_values_i64(handle) {
                const m = mapTable.get(handle);
                if (!m)
                    return allocListI64([]);
                return allocListI64([...m.values()]);
            },
            // --- Test assertions ---
            ...
            // --- Network operations ---
            http_request(methodPtr, urlPtr, headersJsonPtr, bodyPtr) {
            ...
            http_request_full(methodPtr, urlPtr, headersJsonPtr, bodyPtr) {
                const method = readString(methodPtr).toUpperCase();
                const url = readString(urlPtr);
                const headersJson = readString(headersJsonPtr);
                const body = readString(bodyPtr);
            ...
            http_listen(portN) {
                ...
                if (effectErr)
                    return allocResultI64(false, 0n, writeString(effectErr));
                try {
            ...
            http_next_request(handleN) {
                ...
                if (!session)
                    return allocResultString(false, writeString(`Unknown http server handle: ${handle}`));
                ...
                if (state !== 1)
                    return allocResultString(false, writeString("http_next_request: unexpected SAB state"));
            ...
            list_dir(pathPtr) {
                ...
                try {
                    ...
                    return allocListI32(entries.map((e) => writeString(e)));
                }
                catch {
                    return allocListI32([]);
                }
            ...
            fs_watch_next(handleN, timeoutN) {
                const handle = Number(handleN);
            ...
            list_reverse(ptr, elemSize) {
                ...
                for (let i = 0; i < len; i++) {
                    new Uint8Array(memory.buffer, newPtr + 4 + (len - 1 - i) * elemSize, elemSize).set(src.subarray(i * elemSize, (i + 1) * elemSize));
                }
                return newPtr;
            },
            // --- Secret operations ---
            ...
            get_secret(namePtr) {
                ...
                if (effectErr) {
                    audit({ effect: "Secret", op: "get_secret", name, result: "denied", reason: effectErr });
                    return allocOptionI32(null);
                }
            ...
            call_model(modelPtr, promptPtr) {
                const model = readString(modelPtr);
                const prompt = readString(promptPtr);
                const effectErr = policyCheckEffect("Model");
                if (effectErr) {
                    audit({ effect: "Model", op: "call_model", model, result: "denied", reason: effectErr });
                    return allocResultString(false, writeString(effectErr));
                }
                const t0 = Date.now();
            ...
            call_model_system(modelPtr, systemPtr, promptPtr) {
                const model = readString(modelPtr);
                const system = readString(systemPtr);
                const prompt = readString(promptPtr);
                const effectErr = policyCheckEffect("Model");
                if (effectErr) {
                    audit({ effect: "Model", op: "call_model_system", model, result: "denied", reason: effectErr });
                    return allocResultString(false, writeString(effectErr));
                }
                const t0 = Date.now();
            ...
            mcp_connect(urlPtr) {
                ...
                if (effectErr) {
                    audit({ effect: "MCP", op: "mcp_connect", url, result: "denied", reason: effectErr });
                    return allocResultI64(false, 0n, writeString(effectErr));
                }
                const urlErr = policyCheckUrl(url);
                if (urlErr) {
                    audit({ effect: "MCP", op: "mcp_connect", url, result: "denied", reason: urlErr });
                    return allocResultI64(false, 0n, writeString(urlErr));
                }
                const id = mcpNextId++;
            ...
            mcp_call_tool(sessionId, toolPtr, argsPtr) {
                const session = mcpSessions.get(Number(sessionId));
                if (!session) {
                    return allocResultString(false, writeString(`Unknown MCP session: ${sessionId}`));
                }
            ...
            a2a_discover(urlPtr) {
                ...
                if (effectErr) {
                    audit({ effect: "A2A", op: "a2a_discover", url: baseUrl, result: "denied", reason: effectErr });
                    return allocResultString(false, writeString(effectErr));
                }
                const urlErr = policyCheckUrl(baseUrl);
                if (urlErr) {
                    audit({ effect: "A2A", op: "a2a_discover", url: baseUrl, result: "denied", reason: urlErr });
                    return allocResultString(false, writeString(urlErr));
                }
                try {
                    const t0 = Date.now();
                    const resp = syncHttpRequest({
            ...
            a2a_submit(urlPtr, messagePtr) {
                const baseUrl = readString(urlPtr).replace(/\/$/, "");
                const message = readString(messagePtr);
                const effectErr = policyCheckEffect("A2A");
                if (effectErr) {
                    audit({ effect: "A2A", op: "a2a_submit", url: baseUrl, result: "denied", reason: effectErr });
                    return allocResultString(false, writeString(effectErr));
                }
                const urlErr = policyCheckUrl(baseUrl);
                if (urlErr) {
                    audit({ effect: "A2A", op: "a2a_submit", url: baseUrl, result: "denied", reason: urlErr });
                    return allocResultString(false, writeString(urlErr));
                }
            ...
            a2a_poll(urlPtr, taskIdPtr) {
                const baseUrl = readString(urlPtr).replace(/\/$/, "");
                const taskId = readString(taskIdPtr);
                const effectErr = policyCheckEffect("A2A");
                if (effectErr) {
                    audit({ effect: "A2A", op: "a2a_poll", url: baseUrl, task_id: taskId, result: "denied", reason: effectErr });
                    return allocResultString(false, writeString(effectErr));
                }
                const urlErr = policyCheckUrl(baseUrl);
                if (urlErr) {
                    audit({ effect: "A2A", op: "a2a_poll", url: baseUrl, task_id: taskId, result: "denied", reason: urlErr });
                    return allocResultString(false, writeString(urlErr));
                }
                const body = JSON.stringify({
                    jsonrpc: "2.0",
            ...
            a2a_cancel(urlPtr, taskIdPtr) {
                const baseUrl = readString(urlPtr).replace(/\/$/, "");
                const taskId = readString(taskIdPtr);
                const effectErr = policyCheckEffect("A2A");
                if (effectErr) {
                    audit({ effect: "A2A", op: "a2a_cancel", url: baseUrl, task_id: taskId, result: "denied", reason: effectErr });
                    return allocResultString(false, writeString(effectErr));
                }
                const urlErr = policyCheckUrl(baseUrl);
                if (urlErr) {
                    audit({ effect: "A2A", op: "a2a_cancel", url: baseUrl, task_id: taskId, result: "denied", reason: urlErr });
                    return allocResultString(false, writeString(urlErr));
                }
                const body = JSON.stringify({
                    jsonrpc: "2.0",
            ...
            tty_read_key(timeoutN) {
                ...
                if (current === 1 /* KEY_READY */) {
                    ...
                    Atomics.notify(ctrl, 0, 1);
                    return allocOptionI32(writeString(key));
                }
                // EOF, ERROR, or timeout → None
                return allocOptionI32(null);
            },
            tty_cursor_up(nN) {
                const n = Number(nN);
            ...
            // -----------------------------------------------------------------------
            // Mux primitives (multi-stream SSE fan-in)
            // -----------------------------------------------------------------------
            mux_open() {
            ...
            mux_add(handleN, streamIdPtr, urlPtr, headersPtr) {
                const handle = Number(handleN);
                const session = muxTable.get(handle);
                if (!session)
                    return;
                const streamId = readString(streamIdPtr);
                const url = readString(urlPtr);
                const headers = readString(headersPtr);
                const urlErr = policyCheckUrl(url);
                if (urlErr)
                    return;
                const effectErr = policyCheckEffect("Network");
            ...
            mux_remove(handleN, streamIdPtr) {
                const handle = Number(handleN);
                const session = muxTable.get(handle);
                if (!session)
                    return;
            ...
            trace_log(spanIdBig, messagePtr) {
                const spanId = Number(spanIdBig);
                const message = readString(messagePtr);
                const span = spanTable.get(spanId);
                if (span) {
            ...
            // --- Persist operations ---
            checkpoint_save(keyPtr, valuePtr) {
                const key = readString(keyPtr);
            ...
            checkpoint_save_raw(keyPtr, valuePtr) {
                const key = readString(keyPtr);
                const value = readString(valuePtr);
                const effectErr = policyCheckEffect("Persist");
                if (effectErr)
            ...
            cosine_similarity(aPtr, bPtr) {
                try {
                    const a = JSON.parse(readString(aPtr));
                    const b = JSON.parse(readString(bPtr));
            ...
            embed_and_retrieve(queryPtr, chunksJsonPtr, topKN) {
                ...
                try {
                    ...
                    const scores = data.slice(1).map((d, i) => {
                        ...
                        for (let j = 0; j < qVec.length; j++) {
                            dot += qVec[j] * d.embedding[j];
                            nA += qVec[j] * qVec[j];
                            nB += d.embedding[j] * d.embedding[j];
                        }
            ...
            eval_llm_judge(modelPtr, promptPtr, responsePtr, rubricPtr) {
                ...
                if (tag !== 0)
                    return resultPtr; // propagate Err
            ...
            eval_semantic(gotPtr, expectedPtr) {
                const got = readString(gotPtr);
                const expected = readString(expectedPtr);
                const effectErr = policyCheckEffect("Eval");
                if (effectErr)
                ...
                try {
                    ...
                    const model = process.env.CLARITY_EMBED_MODEL ?? "text-embedding-ada-002";
                    if (!apiKey)
                        return allocResultString(false, writeString("OPENAI_API_KEY not set"));
                    const resp = syncHttpRequest({
                        url: `${baseUrl}/v1/embeddings`,
                        method: "POST",
                        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
            ...
            // Streaming builtins
            // -----------------------------------------------------------------------
            stream_start(modelPtr, promptPtr, systemPtr) {
                const model = readString(modelPtr);
                const prompt = readString(promptPtr);
                const system = readString(systemPtr);
                const effectErr = policyCheckEffect("Model");
            ...
            stream_next(handleN) {
                ...
                if (!session)
                    return allocOptionI32(null); // unknown handle → None
            ...
            // -----------------------------------------------------------------------
            sse_next_event_timeout(handleN, timeoutN) {
                const handle = Number(handleN);
                const timeoutMs = Number(timeoutN);
                const session = sseSessions.get(handle);
                if (!session)
    ...
    return {
        ...
        bindMemory,
        // Test runner API
        setCurrentTest(name) { currentTestFunction = name; },

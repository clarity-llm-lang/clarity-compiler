...
export function createAgentRuntime(h, mcpSessions, getMcpNextId, spanTable, getNextSpanId) {
    return {
        // --- Secret operations ---
        get_secret(namePtr) {
            ...
            if (effectErr) {
                h.policyAuditLog({ effect: "Secret", op: "get_secret", name, result: "denied", reason: effectErr });
        ...
        mcp_connect(urlPtr) {
            ...
            if (effectErr) {
                h.policyAuditLog({ effect: "MCP", op: "mcp_connect", url, result: "denied", reason: effectErr });
                return h.allocResultI64(false, 0n, h.writeString(effectErr));
            }
            const urlErr = h.policyCheckUrl(url);
            if (urlErr) {
                h.policyAuditLog({ effect: "MCP", op: "mcp_connect", url, result: "denied", reason: urlErr });
                return h.allocResultI64(false, 0n, h.writeString(urlErr));
            }
        ...
        mcp_list_tools(sessionId) {
            ...
            if (!session)
                return h.allocResultString(false, h.writeString(`Unknown MCP session: ${sessionId}`));
        ...
        a2a_discover(urlPtr) {
            ...
            if (effectErr) {
                h.policyAuditLog({ effect: "A2A", op: "a2a_discover", url: baseUrl, result: "denied", reason: effectErr });
                return h.allocResultString(false, h.writeString(effectErr));
            }
            const urlErr = h.policyCheckUrl(baseUrl);
            if (urlErr) {
                h.policyAuditLog({ effect: "A2A", op: "a2a_discover", url: baseUrl, result: "denied", reason: urlErr });
                return h.allocResultString(false, h.writeString(urlErr));
            }
            try {
                const t0 = Date.now();
        ...
        a2a_submit(urlPtr, messagePtr) {
            ...
            const effectErr = h.policyCheckEffect("A2A");
            if (effectErr) {
                h.policyAuditLog({ effect: "A2A", op: "a2a_submit", url: baseUrl, result: "denied", reason: effectErr });
                return h.allocResultString(false, h.writeString(effectErr));
            }
            const urlErr = h.policyCheckUrl(baseUrl);
            if (urlErr) {
                h.policyAuditLog({ effect: "A2A", op: "a2a_submit", url: baseUrl, result: "denied", reason: urlErr });
                return h.allocResultString(false, h.writeString(urlErr));
            }
        ...
        a2a_poll(urlPtr, taskIdPtr) {
            ...
            const effectErr = h.policyCheckEffect("A2A");
            if (effectErr) {
                h.policyAuditLog({ effect: "A2A", op: "a2a_poll", url: baseUrl, task_id: taskId, result: "denied", reason: effectErr });
                return h.allocResultString(false, h.writeString(effectErr));
            }
            const urlErr = h.policyCheckUrl(baseUrl);
            if (urlErr) {
                h.policyAuditLog({ effect: "A2A", op: "a2a_poll", url: baseUrl, task_id: taskId, result: "denied", reason: urlErr });
                return h.allocResultString(false, h.writeString(urlErr));
            }
        ...
        a2a_cancel(urlPtr, taskIdPtr) {
            ...
            const effectErr = h.policyCheckEffect("A2A");
            if (effectErr) {
                h.policyAuditLog({ effect: "A2A", op: "a2a_cancel", url: baseUrl, task_id: taskId, result: "denied", reason: effectErr });
                return h.allocResultString(false, h.writeString(effectErr));
            }
            const urlErr = h.policyCheckUrl(baseUrl);
            if (urlErr) {
                h.policyAuditLog({ effect: "A2A", op: "a2a_cancel", url: baseUrl, task_id: taskId, result: "denied", reason: urlErr });
                return h.allocResultString(false, h.writeString(urlErr));
            }
        ...
        hitl_ask(keyPtr, questionPtr) {
            ...
            if (effectErr)
                return h.writeString(`[HumanInLoop denied: ${effectErr}]`);

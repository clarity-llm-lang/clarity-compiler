...
req.end();
`;
function callAnthropic(model, messages, h) {
...
function callLLM(model, messages, h) {
    if (model.startsWith("claude-"))
        return callAnthropic(model, messages, h);
    const body = JSON.stringify({ model, messages, max_tokens: 4096 });
    return callOpenAI(body, h);
}
export function createLlmRuntime(h) {
    ...
    return {
        call_model(modelPtr, promptPtr) {
            ...
            if (effectErr) {
                h.policyAuditLog({ effect: "Model", op: "call_model", model, result: "denied", reason: effectErr });
        ...
        call_model_system(modelPtr, systemPtr, promptPtr) {
            ...
            const effectErr = h.policyCheckEffect("Model");
            if (effectErr) {
                h.policyAuditLog({ effect: "Model", op: "call_model_system", model, result: "denied", reason: effectErr });
                return h.allocResultString(false, h.writeString(effectErr));
            }
            const t0 = Date.now();
        ...
        cosine_similarity(aPtr, bPtr) {
            try {
                const a = JSON.parse(h.readString(aPtr));
                const b = JSON.parse(h.readString(bPtr));
        ...
        eval_semantic(gotPtr, expectedPtr) {
            ...
            try {
                ...
                const model = process.env.CLARITY_EMBED_MODEL ?? "text-embedding-ada-002";
                if (!apiKey)
                    return h.allocResultString(false, h.writeString("OPENAI_API_KEY not set"));

...
import { INT64, FLOAT64, STRING, BOOL, UNIT, ERROR_TYPE, typesEqual, typeToString, promoteType, containsTypeVar, substituteTypeVars, unifyTypes, } from "./types.js";
...
export function checkExprInner(ctx, expr) {
    switch (expr.kind) {
        ...
        case "CallExpr": {
            ...
            if (calleeType.kind !== "Function") {
                ctx.diagnostics.push(error(`Cannot call non-function type ${typeToString(calleeType)}`, expr.callee.span));
                return ERROR_TYPE;
            }
            // Check argument count
            if (expr.args.length !== calleeType.params.length) {
                ctx.diagnostics.push(error(`Expected ${calleeType.params.length} arguments but got ${expr.args.length}`, expr.span));
            ...
            if (hasNamedArgs && calleeType.paramNames) {
                ...
                if (hasUnnamedArgs) {
                    ctx.diagnostics.push(error(`Cannot mix named and positional arguments`, expr.span));
                }
                else {
                    ...
                    for (const arg of expr.args) {
                        ...
                        if (idx === -1) {
                            ctx.diagnostics.push(error(`Unknown parameter name '${arg.name}'. Expected one of: ${paramNames.join(", ")}`, arg.span));
                            hasError = true;
                        }
                        else if (reordered[idx] !== undefined) {
                            ctx.diagnostics.push(error(`Duplicate named argument '${arg.name}'`, arg.span));
                            hasError = true;
                        }
                        else {
                            reordered[idx] = arg;
                ...
                ctx.diagnostics.push(error(`Named arguments are not supported for function type values`, expr.span));
            }
            ...
            if (isGeneric) {
                ...
                for (let i = 0; i < expr.args.length; i++) {
                    ...
                    if (!unifyTypes(paramType, argType, bindings)) {
                        ctx.diagnostics.push(error(`Argument ${i + 1}: expected ${typeToString(paramType)} but got ${typeToString(argType)}`, expr.args[i].span));
                    }
                }
                ...
                // Check effects
                ctx.diagnostics.push(...checkEffectSafety(ctx.currentEffects, calleeType.effects, expr.span));
        ...
        case "MemberExpr": {
            ...
            if (objType.kind === "Record") {
                ...
                if (!fieldType) {
                    ctx.diagnostics.push(error(`Record '${objType.name}' has no field '${expr.member}'`, expr.span));
                    return ERROR_TYPE;
                }
                return fieldType;
            }
            ctx.diagnostics.push(error(`Cannot access member '${expr.member}' on type ${typeToString(objType)}`, expr.span));
            return ERROR_TYPE;
        }
        case "MatchExpr": {
            ...
            ctx.diagnostics.push(...checkExhaustiveness(scrutineeType, expr.arms.map((a) => a.pattern), expr.span));
            let resultType = null;
            for (const arm of expr.arms) {
                ...
                if (arm.guard) {
                    ...
                    if (guardType.kind !== "Error" && guardType.kind !== "Bool") {
                        ctx.diagnostics.push(error(`Pattern guard must be Bool, got ${typeToString(guardType)}`, arm.guard.span));
                    }
                }
                ...
                if (resultType === null) {
                    ...
                    ctx.diagnostics.push(error(`Match arm returns ${typeToString(armType)} but previous arm returns ${typeToString(resultType)}`, arm.body.span, "All match arms must return the same type"));
                }
                else {
            ...
            if (expr.typeAnnotation) {
                ...
                if (annotType) {
                    ...
                    if (!containsTypeVar(valueType) && !typesEqual(valueType, annotType)) {
                        ctx.diagnostics.push(error(`Let binding type mismatch: expected ${typeToString(annotType)} but got ${typeToString(valueType)}`, expr.value.span));
                    }
        ...
        case "AssignmentExpr": {
            ...
            if (!sym.mutable) {
                ctx.diagnostics.push(error(`Cannot assign to immutable variable '${expr.name}'`, expr.span, "Declare with 'let mut' to make it mutable"));
                return UNIT;
            }
            const valueType = ctx.checkExpr(expr.value);
            if (!typesEqual(valueType, sym.type)) {
                ctx.diagnostics.push(error(`Cannot assign ${typeToString(valueType)} to variable '${expr.name}' of type ${typeToString(sym.type)}`, expr.value.span));
            }
            return UNIT;
        }
        case "BlockExpr": {
        ...
        case "LambdaExpr": {
            ...
            const paramTypes = expr.params.map(p => ctx.resolveTypeRef(p.typeAnnotation) ?? ERROR_TYPE);
        ...
        case "ListLiteral": {
            ...
            for (let i = 1; i < expr.elements.length; i++) {
                ...
                if (!typesEqual(elemType, firstType)) {
                    ctx.diagnostics.push(error(`List element type mismatch: expected ${typeToString(firstType)} but got ${typeToString(elemType)}`, expr.elements[i].span));
                }
            }
        ...
        case "RecordLiteral": {
            ...
            if (!matchingType) {
                ctx.diagnostics.push(error(`No record type found with fields: ${[...fieldNames].join(", ")}`, expr.span));
                return ERROR_TYPE;
            }
            ...
            for (const [name, expectedType] of matchingType.fields) {
                ...
                if (actualType && !typesEqual(actualType, expectedType) && actualType.kind !== "Error") {
                    ctx.diagnostics.push(error(`Field '${name}' expected type ${typeToString(expectedType)} but got ${typeToString(actualType)}`, expr.fields.find(f => f.name === name).span));
                }
            }
            return matchingType;
        }
...
export function checkPattern(ctx, pattern, expectedType) {
    switch (pattern.kind) {
        ...
        case "LiteralPattern": {
            ...
            if (!typesEqual(litType, expectedType)) {
                ctx.diagnostics.push(error(`Pattern type mismatch: expected ${typeToString(expectedType)} but got ${typeToString(litType)}`, pattern.span));
            }
            break;
        }
        case "RangePattern": {
            if (expectedType.kind !== "Int64") {
                ctx.diagnostics.push(error(`Range patterns only work on Int64, got ${typeToString(expectedType)}`, pattern.span));
            }
            if (pattern.start.value >= pattern.end.value) {
                ctx.diagnostics.push(error(`Invalid range pattern: start (${pattern.start.value}) must be less than end (${pattern.end.value})`, pattern.span));
            }
            break;
        }
        case "ConstructorPattern": {
            if (expectedType.kind === "Union") {
                ...
                if (!variant) {
                    ctx.diagnostics.push(error(`Unknown variant '${pattern.name}' for type '${expectedType.name}'`, pattern.span));
                    return;
                }
                const fieldEntries = [...variant.fields.entries()];
                if (pattern.fields.length !== fieldEntries.length) {
                    ctx.diagnostics.push(error(`Variant '${pattern.name}' has ${fieldEntries.length} field(s) but pattern has ${pattern.fields.length}`, pattern.span));
                    return;
                }
                ...
                ctx.diagnostics.push(error(`Cannot use constructor pattern on non-union type ${typeToString(expectedType)}`, pattern.span));
            }
            break;
        }
    }
}
//# sourceMappingURL=checker-exprs.js.map
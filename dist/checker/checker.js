...
import { INT64, FLOAT64, STRING, BOOL, UNIT, ERROR_TYPE, resolveBuiltinType, typesEqual, typeToString, promoteType, containsTypeVar, substituteTypeVars, unifyTypes, } from "./types.js";
...
export class Checker {
     ...
     */
    checkModule(module, importedSymbols = [], importedTypes = []) {
        this.diagnostics = [];
        ...
        const importSpan = { start: { offset: 0, line: 0, column: 0 }, end: { offset: 0, line: 0, column: 0 }, source: "<import>" };
        ...
        const localFuncNames = new Set(module.declarations
            .filter((d) => d.kind === "FunctionDecl")
            .map((d) => d.name));
        this.warnMutualTailRecursion(module.declarations.filter((d) => d.kind === "FunctionDecl"), localFuncNames);
        return this.diagnostics;
    }
    /** Register variant constructors for an imported union type */
    registerVariantConstructor(variant, unionType, typeName) {
    ...
    findRecordType(fieldNames, fieldTypes) {
        const candidates = [];
    ...
    checkFunctionBody(decl) {
        ...
        if (bodyType && !typesEqual(bodyType, expectedReturn)) {
            if (!containsTypeVar(bodyType) && !containsTypeVar(expectedReturn)) {
                this.diagnostics.push(error(`Function '${decl.name}' returns ${typeToString(bodyType)} but declared return type is ${typeToString(expectedReturn)}`, decl.body.span));
            }
        }
        this.typeParamsInScope = prevTypeParams;
        this.env.exitScope();
    ...
    checkConstDecl(decl) {
        ...
        if (actualType && !typesEqual(actualType, expectedType)) {
            this.diagnostics.push(error(`Constant '${decl.name}' has type ${typeToString(actualType)} but declared type is ${typeToString(expectedType)}`, decl.value.span));
        }
    ...
    tailCallees(expr) {
        ...
        switch (expr.kind) {
            ...
            case "BlockExpr":
                if (expr.result) {
                    for (const n of this.tailCallees(expr.result))
                        result.add(n);
                }
                break;
            case "MatchExpr":
                for (const arm of expr.arms) {
                    for (const n of this.tailCallees(arm.body))
                        result.add(n);
                }
                break;
            default:
    ...
    warnMutualTailRecursion(decls, localFuncNames) {
            ...
            for (const b of aCallees) {
                ...
                if (bCallees.has(decl.name)) {
                    ...
                    if (!warned.has(pairKey)) {
                        warned.add(pairKey);
                        this.diagnostics.push(warning(`Mutual tail recursion between '${decl.name}' and '${b}' cannot be tail-call optimised and may stack-overflow on deep inputs.`, decl.span, `Consider restructuring '${decl.name}' and '${b}' to use an explicit accumulator or a shared helper with an iterative loop.`));
                    }
                }
            }
        }
        ...
        switch (expr.kind) {
            ...
            case "CallExpr": {
                ...
                if (calleeType.kind !== "Function") {
                    this.diagnostics.push(error(`Cannot call non-function type ${typeToString(calleeType)}`, expr.callee.span));
                    return ERROR_TYPE;
                }
                // Check argument count
                if (expr.args.length !== calleeType.params.length) {
                    this.diagnostics.push(error(`Expected ${calleeType.params.length} arguments but got ${expr.args.length}`, expr.span));
                ...
                if (hasNamedArgs && calleeType.paramNames) {
                    ...
                    if (hasUnnamedArgs) {
                        this.diagnostics.push(error(`Cannot mix named and positional arguments`, expr.span));
                    }
                    else {
                        // Validate all names match parameter names and reorder
                        const paramNames = calleeType.paramNames;
                        const reordered = new Array(expr.args.length);
                        let hasError = false;
                        for (const arg of expr.args) {
                            const idx = paramNames.indexOf(arg.name);
                            if (idx === -1) {
                                this.diagnostics.push(error(`Unknown parameter name '${arg.name}'. Expected one of: ${paramNames.join(", ")}`, arg.span));
                                hasError = true;
                            }
                            else if (reordered[idx] !== undefined) {
                                this.diagnostics.push(error(`Duplicate named argument '${arg.name}'`, arg.span));
                                hasError = true;
                            }
                            else {
                                reordered[idx] = arg;
                    ...
                    this.diagnostics.push(error(`Named arguments are not supported for function type values`, expr.span));
                }
                ...
                if (isGeneric) {
                    ...
                    for (let i = 0; i < expr.args.length; i++) {
                        ...
                        if (!unifyTypes(paramType, argType, bindings)) {
                            this.diagnostics.push(error(`Argument ${i + 1}: expected ${typeToString(paramType)} but got ${typeToString(argType)}`, expr.args[i].span));
                        }
                    }
                    ...
                    // Check effects
                    this.diagnostics.push(...checkEffectSafety(this.currentEffects, calleeType.effects, expr.span));
            ...
            case "MemberExpr": {
                ...
                if (objType.kind === "Record") {
                    ...
                    if (!fieldType) {
                        this.diagnostics.push(error(`Record '${objType.name}' has no field '${expr.member}'`, expr.span));
                        return ERROR_TYPE;
                    }
                    return fieldType;
                }
                this.diagnostics.push(error(`Cannot access member '${expr.member}' on type ${typeToString(objType)}`, expr.span));
                return ERROR_TYPE;
            }
            case "MatchExpr": {
                ...
                this.diagnostics.push(...checkExhaustiveness(scrutineeType, expr.arms.map((a) => a.pattern), expr.span));
                let resultType = null;
                for (const arm of expr.arms) {
                    this.env.enterScope();
                    ...
                    if (arm.guard) {
                        ...
                        if (guardType.kind !== "Error" && guardType.kind !== "Bool") {
                            this.diagnostics.push(error(`Pattern guard must be Bool, got ${typeToString(guardType)}`, arm.guard.span));
                        }
                    }
                    ...
                    if (resultType === null) {
                        ...
                        this.diagnostics.push(error(`Match arm returns ${typeToString(armType)} but previous arm returns ${typeToString(resultType)}`, arm.body.span, "All match arms must return the same type"));
                    }
                    else {
                ...
                if (expr.typeAnnotation) {
                    ...
                    if (annotType) {
                        ...
                        if (!containsTypeVar(valueType) && !typesEqual(valueType, annotType)) {
                            this.diagnostics.push(error(`Let binding type mismatch: expected ${typeToString(annotType)} but got ${typeToString(valueType)}`, expr.value.span));
                        }
            ...
            case "AssignmentExpr": {
                ...
                if (!sym.mutable) {
                    this.diagnostics.push(error(`Cannot assign to immutable variable '${expr.name}'`, expr.span, "Declare with 'let mut' to make it mutable"));
                    return UNIT;
                }
                const valueType = this.checkExpr(expr.value);
                if (!typesEqual(valueType, sym.type)) {
                    this.diagnostics.push(error(`Cannot assign ${typeToString(valueType)} to variable '${expr.name}' of type ${typeToString(sym.type)}`, expr.value.span));
                }
                return UNIT;
            }
            case "BlockExpr": {
            ...
            case "LambdaExpr": {
                ...
                const paramTypes = expr.params.map(p => this.resolveTypeRef(p.typeAnnotation) ?? ERROR_TYPE);
            ...
            case "ListLiteral": {
                ...
                for (let i = 1; i < expr.elements.length; i++) {
                    ...
                    if (!typesEqual(elemType, firstType)) {
                        this.diagnostics.push(error(`List element type mismatch: expected ${typeToString(firstType)} but got ${typeToString(elemType)}`, expr.elements[i].span));
                    }
                }
            ...
            case "RecordLiteral": {
                ...
                if (!matchingType) {
                    this.diagnostics.push(error(`No record type found with fields: ${[...fieldNames].join(", ")}`, expr.span));
                    return ERROR_TYPE;
                }
                ...
                for (const [name, expectedType] of matchingType.fields) {
                    ...
                    if (actualType && !typesEqual(actualType, expectedType) && actualType.kind !== "Error") {
                        this.diagnostics.push(error(`Field '${name}' expected type ${typeToString(expectedType)} but got ${typeToString(actualType)}`, expr.fields.find(f => f.name === name).span));
                    }
                }
                return matchingType;
            }
        ...
        switch (pattern.kind) {
            ...
            case "LiteralPattern": {
                ...
                if (!typesEqual(litType, expectedType)) {
                    this.diagnostics.push(error(`Pattern type mismatch: expected ${typeToString(expectedType)} but got ${typeToString(litType)}`, pattern.span));
                }
                break;
            }
            case "RangePattern": {
                if (expectedType.kind !== "Int64") {
                    this.diagnostics.push(error(`Range patterns only work on Int64, got ${typeToString(expectedType)}`, pattern.span));
                }
                if (pattern.start.value >= pattern.end.value) {
                    this.diagnostics.push(error(`Invalid range pattern: start (${pattern.start.value}) must be less than end (${pattern.end.value})`, pattern.span));
                }
                break;
            }
            case "ConstructorPattern": {
                if (expectedType.kind === "Union") {
                    ...
                    if (!variant) {
                        this.diagnostics.push(error(`Unknown variant '${pattern.name}' for type '${expectedType.name}'`, pattern.span));
                        return;
                    }
                    const fieldEntries = [...variant.fields.entries()];
                    if (pattern.fields.length !== fieldEntries.length) {
                        this.diagnostics.push(error(`Variant '${pattern.name}' has ${fieldEntries.length} field(s) but pattern has ${pattern.fields.length}`, pattern.span));
                        return;
                    }
                    ...
                    this.diagnostics.push(error(`Cannot use constructor pattern on non-union type ${typeToString(expectedType)}`, pattern.span));
                }
                break;
            }
        }
    ...
    // Operator Checking
    // ============================================================
    checkBinaryOp(op, left, right, span) {
        if (left.kind === "Error" || right.kind === "Error")
            return ERROR_TYPE;
        // Arithmetic: +, -, *, /, %
        if (["+", "-", "*", "/", "%"].includes(op)) {
            if (left.kind === "Int64" && right.kind === "Int64")
                return INT64;
            if (left.kind === "Float64" && right.kind === "Float64")
                return FLOAT64;
            // Allow Int64 + Float64 -> Float64
            if ((left.kind === "Int64" && right.kind === "Float64") ||
                (left.kind === "Float64" && right.kind === "Int64")) {
                this.diagnostics.push(error(`Cannot mix Int64 and Float64 in arithmetic`, span, "Clarity does not have implicit numeric conversions. Convert explicitly."));
                return ERROR_TYPE;
            }
            this.diagnostics.push(error(`Operator '${op}' requires numeric types, got ${typeToString(left)} and ${typeToString(right)}`, span));
            return ERROR_TYPE;
        }
        // String concat: ++
        if (op === "++") {
            if (left.kind === "String" && right.kind === "String")
                return STRING;
            this.diagnostics.push(error(`Operator '++' requires String types, got ${typeToString(left)} and ${typeToString(right)}`, span));
            return ERROR_TYPE;
        }
        // Comparison: ==, !=, <, >, <=, >=
        if (["==", "!=", "<", ">", "<=", ">="].includes(op)) {
            if (!typesEqual(left, right)) {
                this.diagnostics.push(error(`Cannot compare ${typeToString(left)} and ${typeToString(right)}`, span, "Both sides of a comparison must have the same type"));
                return ERROR_TYPE;
            }
            return BOOL;
        }
        ...
        if (op === "and" || op === "or") {
            if (left.kind === "Bool" && right.kind === "Bool")
                return BOOL;
            this.diagnostics.push(error(`Operator '${op}' requires Bool types, got ${typeToString(left)} and ${typeToString(right)}`, span));
            return ERROR_TYPE;
        }
        this.diagnostics.push(error(`Unknown operator '${op}'`, span));
        return ERROR_TYPE;
    }
    checkUnaryOp(op, operand, span) {
        if (operand.kind === "Error")
            return ERROR_TYPE;
        if (op === "-") {
            if (operand.kind === "Int64")
                return INT64;
            if (operand.kind === "Float64")
                return FLOAT64;
            this.diagnostics.push(error(`Unary '-' requires numeric type, got ${typeToString(operand)}`, span));
            return ERROR_TYPE;
        }
        if (op === "!") {
            if (operand.kind === "Bool")
                return BOOL;
            this.diagnostics.push(error(`Unary '!' requires Bool type, got ${typeToString(operand)}`, span));
            return ERROR_TYPE;
        }
        return ERROR_TYPE;
    }
...
// ---------------------------------------------------------------------------
function collectFreeVars(expr, bound) {
    const free = new Set();
    walkExpr(expr, bound, free);
    return free;
}
function walkExpr(expr, bound, free) {
    switch (expr.kind) {
        case "IdentifierExpr":
...
function collectPatternBindings(pattern, bound) {
    switch (pattern.kind) {
        case "BindingPattern":

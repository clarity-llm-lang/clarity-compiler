...
export function registerLintCommand(program) {
    program
        .command("lint [file]")
        .description("Lint a .clarity source file for unused declarations and common style issues.\n" +
        ...
        if (ast && Array.isArray(ast.declarations)) {
            ...
            for (const decl of ast.declarations) {
                ...
                if (name.startsWith("test_"))
                    continue; // test functions are entry points
                if (ENTRY_NAMES.has(name))
                    continue; // well-known runtime entry points
                const exported = !!(d["exported"] ?? d["export"]);
                if (exported)
                    continue; // exported functions are used externally
                if (!allRefs.has(name)) {
                    const span = d["span"] ?? {};

...
export function registerWatchCommand(program) {
    program
        .command("watch [file]")
        .description("Watch a .clarity file (and its imports) for changes and re-compile automatically.\n" +
        ...
        const outFile = opts.output ?? absFile.replace(/\.clarity$/, ".wasm");
        let debounceTimer = null;
        let running = false;
        async function recompile(trigger) {
            if (running)
                return;
            running = true;
        ...
        const watcher = nodeWatch(watchDir, { recursive: false }, (_event, filename) => {
            if (!filename)
                return;
            const changed = path.join(watchDir, filename);
            // Re-compile on .clarity file changes in the same directory
            if (!changed.endsWith(".clarity"))
                return;
            if (debounceTimer)

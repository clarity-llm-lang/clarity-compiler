...
export class Lexer {
    ...
    readNumber() {
        ...
        return this.makeToken(isFloat ? TokenKind.FloatLiteral : TokenKind.IntLiteral, value, startPos, startLine, startCol);
    }
    readIdentOrKeyword() {
        const startPos = this.pos;
        const startLine = this.line;
    ...
    readTripleQuoteString(startPos, startLine, startCol) {
        let value = "";
        // Skip the first newline after opening """ if present
        if (this.pos < this.source.length && this.source[this.pos] === "\n") {
            this.advance();
        }
        else if (this.pos + 1 < this.source.length &&
    ...
    readPunctuation() {
        ...
        switch (ch + next) {
            case "->":
                this.advance();
                this.advance();
                return this.makeToken(TokenKind.Arrow, "->", startPos, startLine, startCol);
            case "++":
                this.advance();
                this.advance();
                return this.makeToken(TokenKind.PlusPlus, "++", startPos, startLine, startCol);
            case "==":
                this.advance();
                this.advance();
                return this.makeToken(TokenKind.EqEq, "==", startPos, startLine, startCol);
            case "!=":
                this.advance();
                this.advance();
                return this.makeToken(TokenKind.NotEq, "!=", startPos, startLine, startCol);
            case "<=":
                this.advance();
                this.advance();
                return this.makeToken(TokenKind.LtEq, "<=", startPos, startLine, startCol);
            case ">=":
                this.advance();
                this.advance();
                return this.makeToken(TokenKind.GtEq, ">=", startPos, startLine, startCol);
            case "..":
                this.advance();
                this.advance();
                return this.makeToken(TokenKind.DotDot, "..", startPos, startLine, startCol);
        }
    ...
    makeToken(kind, value, startPos, startLine, startCol) {
        return {
            kind,
            value,
            span: {

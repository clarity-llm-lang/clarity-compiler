...
async function resolveDefaultFile(file) {
    if (file)
        return file;
...
async function emitAgentEvent(daemonUrl, authToken, event) {
    try {
        const headers = {
...
program
    ...
    try {
        ...
        if (opts.emitAst && result.ast) {
            console.log(JSON.stringify(result.ast, (_key, value) => typeof value === "bigint" ? value.toString() : value, 2));
            return;
        }
        ...
        if (result.wasm) {
            const outputFile = opts.output ?? file.replace(/\.clarity$/, ".wasm");
...
program
    ...
    try {
        ...
        if (heapBase && typeof heapBase.value === "number") {
            runtime.setHeapBase(heapBase.value);
        }
        const fnName = opts.function;
...
program
    .command("start [file]")
    .description("Register and start a .clarity service in Clarity Runtime.\n" +
    ...
    const authToken = opts.authToken ?? process.env.CLARITYD_AUTH_TOKEN ?? process.env.CLARITY_API_TOKEN;
    ...
    try {
        ...
        if (meta.module)
            args.push("--module", meta.module);
        if (opts.output)
            args.push("--wasm", path.resolve(String(opts.output)));
        if (meta.entry)
            args.push("--entry", meta.entry);
        if (meta.name)
            args.push("--name", meta.name);
        args.push("--service-type", serviceType);
        if (meta.agent?.id)
            args.push("--agent-id", meta.agent.id);
        if (meta.agent?.name)
            args.push("--agent-name", meta.agent.name);
        if (meta.agent?.role)
            args.push("--agent-role", meta.agent.role);
        if (meta.agent?.objective)
            args.push("--agent-objective", meta.agent.objective);
        if (meta.agent?.inputs?.length)
            args.push("--agent-inputs", meta.agent.inputs.join(","));
        if (meta.agent?.outputs?.length)
            args.push("--agent-outputs", meta.agent.outputs.join(","));
        if (meta.agent?.mcp_tools?.length)
            args.push("--agent-mcp-tools", meta.agent.mcp_tools.join(","));
        if (meta.agent?.llm_providers?.length)
            args.push("--agent-llm-providers", meta.agent.llm_providers.join(","));
        if (meta.agent?.handoff_targets?.length)
            args.push("--agent-handoff-targets", meta.agent.handoff_targets.join(","));
        if (meta.agent?.depends_on?.length)
...
program
    ...
    try {
        ...
        for (const testFn of testFunctions) {
            ...
            try {
                const fn = instance.exports[testFn.name];
                fn();
            }
            catch (e) {
...
function generateFixHint(failure) {
    switch (failure.kind) {
        case "assert_eq":
...
program
    ...
    while (true) {
        ...
        if (isDecl) {
            ...
            function guessReplTypes(expr) {
                ...
                if (hasQuote || hasConcat)
                    return ["String", "Bool", "Int64", "Float64", "Unit", "Bytes", "Timestamp"];
                if (hasBool || (hasCmp && !hasFloat))
                    return ["Bool", "Int64", "Float64", "String", "Unit", "Bytes", "Timestamp"];
                if (hasFloat)
                    return ["Float64", "Int64", "Bool", "String", "Unit", "Bytes", "Timestamp"];
    ...
    rl.close();
    process.exit(0);
    async function runReplEval(wasm, fnName) {
        try {
            const runtime = createRuntime();
            ...
            const fn = instance.exports[fnName];
            if (!fn) {
                console.error("(eval function not found)");
                return;
            }
            const result = fn();
            if (result === undefined)
...
program
    .command("pack [file]")
    .description("Compile a .clarity file and bundle it as a standalone Node.js launcher script.\n" +
    ...
    try {
        ...
        const entryFn = opts.function;
        const wasmB64 = Buffer.from(result.wasm).toString("base64");
        const outFile = opts.output ?? file.replace(/\.clarity$/, ".js");
...
`;
            ...
            console.log(`Packed ${file} -> ${outFile} + ${path.basename(rtDir)}/  ` +
        ...
        const requiredVersion = pkgJson.version;
...
`;
        ...
        console.log(`Packed ${file} -> ${outFile}  ` +

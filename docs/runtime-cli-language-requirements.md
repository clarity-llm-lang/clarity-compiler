...
| `LANG-RUNTIME-WASM-MARSHAL-001` | `LLM-runtime` | P2 | Open | Support structured record/list argument and result marshalling for runtime-invoked local wasm exports so handlers do not need raw JSON strings for `context.v1`, timer payloads, or future A2A envelopes. | `LLM-runtime/docs/requirements/local-wasm-agent-parity-requirements.md` |
